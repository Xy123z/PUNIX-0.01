...
int main(int argc, char** argv) {
    ...
    uint32_t getty_file = copy_host_file(sbin_id, "getty", "getty.prog", 0755);
    if (getty_file == 0) printf("  WARNING: getty.prog not found\n");

    uint32_t login_file = copy_host_file(sbin_id, "login", "login.prog", 0755);
    if (login_file == 0) printf("  WARNING: login.prog not found\n");
    ...
    for (int i = 0; i < 4; i++) {
        ...
        tnode.mode = 0020666; // S_IFCHR | 0666
        tnode.link_count = 1;
        tnode.indirect_block = (4 << 8) | i; // Major 4, Minor i
        tnode.atime = tnode.mtime = tnode.ctime = (uint32_t)time(NULL);
        write_inode(tid, &tnode);
        ...
        int b_idx = -1, e_idx = -1;
        for (int b = 0; b < 12; b++) {
            if (dev_inode.blocks[b] == 0) {
                dev_inode.blocks[b] = block_alloc();
                dev_inode.block_count++;
                memset(entries, 0, SECTOR_SIZE);
            } else {
                read_sector(dev_inode.blocks[b], entries);
            }
            for (int j = 0; j < (SECTOR_SIZE / sizeof(fs_dirent_t)); j++) {
                if (entries[j].inode_id == 0) { b_idx = b; e_idx = j; break; }
            }
            if (b_idx != -1) break;
        }
        entries[e_idx].inode_id = tid;
        strcpy(entries[e_idx].name, tty_name);
        write_sector(dev_inode.blocks[b_idx], entries);
        dev_inode.size += sizeof(fs_dirent_t);
        write_inode(dev_id, &dev_inode);
        
        printf("  Created /dev/%s (Major 4, Minor %d)\n", tty_name, i);
    }
