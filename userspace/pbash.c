...
void execute_pipeline(char* cmd) {
    ...
    for (int i = 0; i < num_cmds; i++) {
        ...
        if (pid == 0) {
            ...
            // Set as foreground PGID of the TTY
            if (i == 0) ioctl(0, TIOCSPGRP, &pipeline_pgid);
    ...
    if (pipeline_pgid > 0) ioctl(0, TIOCSPGRP, &pipeline_pgid);

    // Wait for foreground job
    for (int i = 0; i < num_cmds; i++) {
        int status;
        // WUNTRACED = 2 — told by our new wait.h
        int wait_ret = sys_wait(pids[i], &status);
